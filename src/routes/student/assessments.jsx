...
function AssessmentsPage() {
  ...
  const studentAssessments = (assessments || []).filter((a) => {
    return (
      a.status === "published" &&
      a.batches &&
      a.batches.some((bId) => currentUser.batches?.includes(bId))
    );
  });
        ...
        <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-800">
          ...
          {(searchQuery ||
            ...
            <button
              ...
              className="text-xs font-bold text-[#6C1D5F] dark:text-purple-400 hover:underline cursor-pointer flex items-center gap-1 bg-[#6C1D5F]/10 dark:bg-purple-400/10 px-2 py-1 rounded-md"
            >
            ...
            <table className="w-full text-left border-collapse text-sm whitespace-nowrap min-w-[600px]">
              ...
              <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800/60 bg-white dark:bg-neutral-900">
                {filteredAssessments.map((as) => {
                  const statusColor = {
                    Active:
                      "bg-emerald-100 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800",
                    Upcoming:
                  ...
                  return (
                    <tr
                      key={as.id}
                      className="even:bg-neutral-50/50 dark:even:bg-neutral-800/20 hover:bg-neutral-100 dark:hover:bg-neutral-800/40 transition-colors"
                    >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
          {filteredAssessments.map((as) => {
            const difficultyColor =
              {
                ...
                Medium:
                  "text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/20 border-amber-100 dark:border-amber-900/50",
            ...
            return (
              <div
                key={as.id}
                className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-4 md:p-5 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col group overflow-hidden relative"
              >
                ...
                <div className="flex justify-between items-start gap-3 mb-4">
                  <div className="flex-1 min-w-0">
                    <h3 className="font-black text-sm md:text-base text-neutral-800 dark:text-white leading-tight mb-1 truncate">
                      {as.title}
                    </h3>
                ...
                <div className="mt-auto pt-4 border-t border-neutral-100 dark:border-neutral-800 relative z-10">
                  {as.computedStatus === "Active" ? (
                    ...
                    >
                      ...
                      <span>
                        Starts{" "}
                        {new Date(`${as.startDate}T${as.startTime || "00:00"}`).toLocaleDateString(
                          "en-GB",
                          {
                            day: "numeric",
                            month: "short",
